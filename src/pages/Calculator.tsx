...
export const CalculatorPage = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchBankInfo = async () => {
      try {
        ...
        setFirestoreError('No se pudo obtener la información de configuración. Intenta más tarde.');
        console.error(error);
      }
    };
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFirestoreError('No se pudo guardar la información del contacto. Intenta más tarde.');
      console.error(error);
      setShowResults(true);
    }
  };
  ...
  const handleUploadReceipt = async () => {
    ...
    try {
      ...
      setFirestoreError('No se pudo enviar el comprobante. Intenta más tarde.');
      console.error(error);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const images = await getCalculatorBannerImages();
        setBannerImages(images);
      } catch (error) {
        setFirestoreError('No se pudieron cargar las imágenes del banner. Intenta más tarde.');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg">
      <div className="flex-1 pt-24 pb-20 px-4 md:px-8 max-w-7xl mx-auto w-full">
        {bannerImages.length > 0 && <CalculatorBanner images={bannerImages} />}
        {firestoreError && (
          <div className="mb-4 p-4 bg-red-900/80 border border-red-500 text-red-200 rounded-xl text-center font-bold">
            {firestoreError}
          </div>
        )}
